...
new_column_titles = {
    ...
    # Add other column titles as needed
}
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
            ...
            ], className="area-controls-box", style={'border': '1px solid grey', 'border-radius': '5px', 'margin-bottom':'80px'})
        ...
        html.Div([
            ...
            ], className="scatter-controls-box", style={'border': '1px solid grey', 'border-radius': '5px', 'margin-bottom':'70px'})
        ], className="six columns"),
    ], className="row"),
    ...
    html.Div([
        ...
        html.Div([
            html.Label("Select users:", className="dropdown-label"),
            dcc.Dropdown(
                id='id-dropdown',
            ...
            dcc.RangeSlider(
                id='date-slider',
                min=df_table['Date'].min().timestamp(),  # Convert min date to timestamp
                max=df_table['Date'].max().timestamp(),  # Convert max date to timestamp
                value=[df_table['Date'].min().timestamp(), df_table['Date'].max().timestamp()],  # Initial range
                marks={timestamp: date.strftime('%Y-%m-%d') for timestamp, date in zip(
                    [df_table['Date'].min().timestamp(), df_table['Date'].max().timestamp()],
                    [df_table['Date'].min(), df_table['Date'].max()]
                )},  # Set marks to display dates
                step=None,  # Allow arbitrary selection of dates
            ),
            html.Label("Step Total Range:", className="slider-label"),
            dcc.RangeSlider(
                id='steptotal-slider',
            ...
            html.Label("Calories Range:", className="slider-label"),
            dcc.RangeSlider(
                id='calories-slider',
            ...
            html.Label("Weight (lbs) Range:", className="slider-label"),
            dcc.RangeSlider(
                id='weight-slider',
            ...
            html.Label("BMI Range:", className="slider-label"),
            dcc.RangeSlider(
                id='bmi-slider',
            ...
            html.Label("Time Asleep Range:", className="slider-label"),
            dcc.RangeSlider(
                id='sleep-slider',
...
def update_table(selected_ids, date_range, step_range, calories_range, weight_range, bmi_range, sleep_range):
    ...
    if triggered_input == 'id-dropdown':
        ...
        filtered_df = df_table[(df_table['Date'] >= pd.Timestamp(date_range[0]).date()) & 
                               (df_table['Date'] <= pd.Timestamp(date_range[1]).date())]
        return filtered_df.to_dict('records')

    elif triggered_input in ['steptotal-slider', 'calories-slider', 'weight-slider', 'bmi-slider', 'sleep-slider']:
        # Filter the dataframe based on selected ranges for different attributes and return the filtered data
        filtered_df = df_table[(df_table['StepTotal'] >= step_range[0]) & (df_table['StepTotal'] <= step_range[1]) &
                               (df_table['Calories'] >= calories_range[0]) & (df_table['Calories'] <= calories_range[1]) &
                               (df_table['WeightPounds'] >= weight_range[0]) & (df_table['WeightPounds'] <= weight_range[1]) &
                               (df_table['BMI'] >= bmi_range[0]) & (df_table['BMI'] <= bmi_range[1]) &
                               (df_table['TotalMinutesAsleep'] >= sleep_range[0]) & (df_table['TotalMinutesAsleep'] <= sleep_range[1])]
        return filtered_df.to_dict('records')
